                             ...
                             float *filtered_image)
{
    ...
    {
        if (i + pixel_row - patch_size / 2 >= 0 && i + pixel_row - patch_size / 2 < n)
        {
            patches[pixel_col + i * n] = image[pixel_col + (i + pixel_row - patch_size / 2) * n];
        }
    }
    ...
    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < n; j++)
        {
            dist = util::cuda_compute_patch_distance(image,
                                                     weights,
                                                     n,
                                                     patch_size,
                                                     patch_row_start,
...
__host__ std::vector<float> filter_image(float * image,
                                         int n,
                                         int patch_size,
                                         float patch_sigma,
                                         float filter_sigma)
{
    ...
    int size_shared_memory = n * patch_size * sizeof(float);
    ...
    filter_pixel<<<n, n, size_shared_memory>>>(d_image, d_weights, n, patch_size, filter_sigma, d_res);
